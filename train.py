"""Train a glioma IDH-status classifier."""
import argparse, json
from src.idh_model import make_demo, prepare_data, read_table, train_and_save
def main():
    ...
    p.add_argument("--sample-column", default="sample_id"); p.add_argument("--target-column", default="idh_status"); p.add_argument("--positive-label")
    p.add_argument("--output-dir", default="outputs/run"); p.add_argument("--test-size", type=float, default=.25); p.add_argument("--top-k", type=int, default=75); p.add_argument("--seed", type=int, default=42); a = p.parse_args()
    # Demo mode verifies the full pipeline without downloading TCGA data.
