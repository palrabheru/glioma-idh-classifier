"""Comprehensive glioma IDH model comparison and reporting workflow."""
...
from .idh_model import SafeSelectKBest
...
def _save_eda(X: pd.DataFrame, y: pd.Series, out: Path) -> None:
    ...
    plt.figure(figsize=(5, 4)); plt.bar(["IDH wild-type", "IDH mutant"], counts.reindex([0, 1], fill_value=0), color=["#64748b", "#7e22ce"])
    plt.ylabel("Samples"); plt.title("IDH class distribution"); plt.tight_layout(); plt.savefig(figures / "class_distribution.png", dpi=180); plt.close()
    ...
    for label, name, color in [(0, "IDH wild-type", "#64748b"), (1, "IDH mutant", "#7e22ce")]:
        mask = y.to_numpy() == label; plt.scatter(pca[mask, 0], pca[mask, 1], s=28, alpha=.75, label=name, c=color)
    plt.xlabel("PC1"); plt.ylabel("PC2"); plt.title("PCA by IDH status"); plt.legend(); plt.tight_layout(); plt.savefig(figures / "pca_by_idh_status.png", dpi=180); plt.close()
...
def run_full_analysis(X: pd.DataFrame, y: pd.Series, ids: pd.Series, output_dir,
    ...
    report = f"""# Glioma IDH Classification Report
...
- IDH labels, glioma grades, and patient identifiers must be harmonized before combining cohorts.
"""
