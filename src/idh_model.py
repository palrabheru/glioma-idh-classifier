"""Leakage-safe IDH-status classification from gene expression."""
...
POSITIVE_LABELS = {"1", "true", "yes", "mutant", "mutated", "idh-mutant", "idh_mutant"}
...
def encode_target(series, positive_label=None):
    """Convert a two-class IDH endpoint to 0 (wild-type) and 1 (mutant)."""
    ...
    if positive not in unique: raise ValueError("Cannot identify mutant class; pass --positive-label.")
    return (values == positive).astype(int)

def prepare_data(expression, clinical, sample_column, target_column, positive_label=None):
    """Join expression and clinical tables and validate model requirements."""
...
def make_demo(seed=42, n_samples=180, n_genes=150):
    """Create deterministic synthetic data for installation checks and tutorials."""
    informative = min(18, max(2, n_genes // 2)); redundant = min(10, max(0, n_genes - informative - 1))
    X, y = make_classification(n_samples=n_samples, n_features=n_genes, n_informative=informative, n_redundant=redundant, weights=[.35, .65], class_sep=1.4, random_state=seed)
    return pd.DataFrame(X, columns=[f"GENE_{i:03d}" for i in range(n_genes)]), pd.Series(y), pd.Series([f"GLIOMA_{i:03d}" for i in range(n_samples)])

def train_and_save(X, y, ids, output_dir, test_size=.25, k=75, seed=42):
    ...
    # Keep every learned preprocessing step inside the pipeline to prevent the
    # held-out test samples from influencing gene selection or normalization.
    ...
    metrics = {"n_samples": len(X), "n_genes": X.shape[1], "n_test": len(test), "mutant_rate": y.mean(),
    ...
    pd.DataFrame({"sample_id": ids.iloc[test].to_numpy(), "true_label": truth, "predicted_label": prediction, "mutant_probability": probability}).to_csv(out / "predictions.csv", index=False)
    # Translate the two sequential feature masks back to original gene names.
    ...
    ConfusionMatrixDisplay(confusion_matrix(truth, prediction), display_labels=["wild-type", "mutant"]).plot(cmap="Purples", colorbar=False)
