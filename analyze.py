"""Run the complete glioma IDH model comparison and report."""
import argparse, json
from src.full_analysis import run_full_analysis
from src.idh_model import make_demo, prepare_data, read_table
...
def main():
    ...
    p.add_argument("--target-column", default="idh_status"); p.add_argument("--positive-label"); p.add_argument("--output-dir", default="results/full_analysis")
    p.add_argument("--test-size", type=float, default=.25); p.add_argument("--top-k", type=int, default=75); p.add_argument("--seed", type=int, default=42); a = p.parse_args()
